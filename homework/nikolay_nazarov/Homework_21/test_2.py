...
class TestTask:
    ...
    def test_alphabetical_sorting_and_quality_control(self):
        ...
        sort = Select(self.driver.find_element(By.ID, 'selectProductSort'))
    ...
    def test_button_click(self):
        ...
        start_button = self.driver.find_element(By.ID, 'button00')
        start_button.click()
        first_button = self.driver.find_element(By.ID, 'button01')
        first_button.click()
        second_button = self.driver.find_element(By.ID, 'button02')
        second_button.click()
        third_button = self.driver.find_element(By.ID, 'button03')
        third_button.click()
        check_button = self.driver.find_element(By.ID, 'buttonmessage')
    ...
    def test_button_click_task_2(self):
        self.driver.get('https://testpages.herokuapp.com/styled/dynamic-buttons-simple.html')
        start_button = self.driver.find_element(By.ID, 'button00')
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(start_button))
        start_button.click()
        first_button = self.driver.find_element(By.ID, 'button01')
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(first_button))
        first_button.click()
        second_button = self.driver.find_element(By.ID, 'button02')
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(second_button))
        second_button.click()
        third_button = self.driver.find_element(By.ID, 'button03')
        WebDriverWait(self.driver, 10).until(EC.element_to_be_clickable(third_button))
        third_button.click()
        check_button = self.driver.find_element(By.ID, 'buttonmessage')
        assert 'All Buttons Clicked' in check_button.text
